...
File <- file.path("C:/Users/Daman Kaur/Desktop/Ulster/Research Projects")
setwd(File)
#nacc <- fread("kaur10262020.csv")
nacc <- fread("nacc62_2023update.csv")
...
ds <- nacc[,c(76:115)]
dc <- subset(nacc, select = c(CVHATT,CVAFIB,CVANGIO,CVBYPASS,CVPACDEF,CVPACE,CVCHF,CVANGINA,CVHVALVE,CVOTHR,
                              MYOINF,CONGHRT,AFIBRILL,ANGINA,ANGIOCP,ANGIOPCI,PACEMAKE,HVALVE,NACCAAAS,NACCAANX,
                              NACCAC,NACCACEI,NACCADEP,NACCADMD,NACCAHTN,NACCANGI,NACCAPSY,NACCBETA,NACCCCBS,NACCDBMD,
                              NACCDIUR,NACCHTNC,NACCLIPL,NACCNSD,NACCPDMD,NACCVASD))
ds <- as.data.frame(ds)
dc <- as.data.frame(dc)

sub.nacc <- cbind(sub.nacc,ds,dc)
sub.nacc <- as.data.frame(sub.nacc)
...
write.csv(df, file = "NtoDudsdcox.csv",row.names=FALSE, na="")
...
df$Diagnosis[(df$Diagnosis == 'TRUE' )]<- 1
df <- df[,-(129)] 
write.csv(df, file = "NtoMudsdcox.csv",row.names=FALSE, na="")
...
df$Diagnosis[(df$Diagnosis == 'TRUE' )]<- 1
df <- df[,-(129)] 
write.csv(df, file = "MtoDudsdcox.csv",row.names=FALSE, na="")
