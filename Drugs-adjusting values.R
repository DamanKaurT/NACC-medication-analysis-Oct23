...
### Only Keep required observations (can remove the 1st MCI (from NtoM) or 1st dementia (from NtoD/MtoD) visit)
...
write.csv(df, file = "MDallmedscdrcox.csv",row.names=FALSE, na="")
...
write.csv(df, file = "MDhypercdrcox.csv",row.names=FALSE, na="")
